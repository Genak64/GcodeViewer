...
//	console.log(shape);
	...
	return {
		getShape: function (items){
		return parseCadres(items);
		}
	}
}());
...
const shapeToGcode=(function (){
	
	function createGcode(shape){
		var gcode=[];
...
//		console.log("points["+0+"] x="+path.points[0].x+" y="+path.points[0].y);
		partGcode.push("M4 S"+path.power);
		partGcode.push("G1"+"X"+path.points[1].x.toString()+"Y"+path.points[1].y.toString()+"F"+path.speed);
//		console.log("points["+1+"] x="+path.points[1].x+" y="+path.points[1].y);
		for (i=2;i<path.points.length;i++){
//			console.log("points["+i+"] x="+path.points[i].x+" y="+path.points[i].y);
	...
	return {
		getGcode: function (shape){
		return createGcode(shape);
	}
	}
